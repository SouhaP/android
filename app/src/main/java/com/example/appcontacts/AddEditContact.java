...
//action bar
    ...
    //add event to save data
     ...
     //   groupList.add(new GroupModel("2", "Friends", "Friends circle", "time1", "time2"));
        ...
        if (isEditMode){
            //set toolbar title
            actionBar.setTitle("Modifier un Contact");
        ...
        if ( resultCode == RESULT_OK && data != null) {
            switch (requestCode) {
                case REQUEST_IMAGE_FROM_GALLERY:
                    ...
                    modifyAndDisplayImage(imageUri);
                    break;
...
//check edit or add mode to save data in sql
        if (isEditMode){
            ...
            Toast.makeText(getApplicationContext(), "Modifié avec succès...", Toast.LENGTH_SHORT).show();
            ...
            Toast.makeText(getApplicationContext(), "Inséré avec succès..." + groupId, Toast.LENGTH_SHORT).show();
            Intent intent = new Intent(this, MainActivity.class);
            startActivity(intent);
            finish();
        }
        ...
        // show toast message
        Toast.makeText(getApplicationContext(), "Rien à sauvegarder...", Toast.LENGTH_SHORT).show();
    }
